...
def self_play(env, config: MuZeroConfig, replay_buffer: ReplayBuffer, network: CartPoleNetwork, filename):
    ...
    for i in range(config.num_epochs):  # Number of Steps of train/play alternations
        ...
        if reward_avg >= CARTPOLE_STOP_REWARD:
            ...
            # Plotting code here
            train_results.plot_individual_losses(filename)
            train_results.plot_total_loss(filename)
            test_rewards.plot_rewards(filename)
            return
